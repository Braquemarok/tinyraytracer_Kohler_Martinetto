...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    for (int i=0; i < duck.nfaces() ; i++) {
        float dist_i;
        if (duck.ray_triangle_intersect(i,orig, dir, dist_i) && dist_i < duck_dist && dist_i < spheres_dist && dist_i < checkerboard_dist) {
            duck_dist = dist_i;
            hit = orig + dir*dist_i;
...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth=0) {
    ...
    if (depth>4 || !scene_intersect(orig, dir, spheres, point, N, material)) {
        ...
        float r=sqrt(dir*dir);
        float t=acos((dir.y)/r);
        float p=atan2((dir.z), (dir.x));

        int i = ((p+M_PI)/(2*M_PI))*(envmap_width-1);
